...
const clone = () => new Promise((fulfill, reject) => {
    ...
    exec(config.outputDir, [
        'git clone "' + config.repo + '" --branch master "' + config.outputDir + '"',
...
module.exports = branch => new Promise((fulfill, reject) => {
    clone().then(() => {
        // Check branch
        exec(config.outputDir, ['git branch'], output => {
            let result = output.shift().toString('utf8').split('\n').filter(item => item.indexOf('*') !== -1);
            if (result.length !== 1) {
            ...
            // Change branch
            exec(config.outputDir, ['git checkout ' + branch], output => {
