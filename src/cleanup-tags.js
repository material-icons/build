    ...
    function removeGroups($parent) {
        ...
        $groups.forEach($group => {
            ...
            Object.keys(group.attribs ? group.attribs : {}).forEach(prop => {
                ...
                switch (prop) {
                    case 'fill':
                        if (value === '#000' || value === '#000000' || value === 'black' || value === '#010101') {
                            break;
                        }
                        if (value !== 'none') {
                    ...
                    default:
                        console.log(svg.toString());
                        throw new Error('Weird group attribute: ' + prop + '="' + value + '"');
                    ...
                    if (props) {
                        if (props.fill !== void 0) {
                            switch (props.fill) {
                                ...
                                default:
                                    console.log(svg.toString());
                                    throw new Error('Invalid fill for path: ' + props.fill + ' in ' + key);
                        ...
                        ['opacity', 'fill-opacity'].forEach(attr => {
                            ...
                            if (value > 0.85) {
                                console.log('Changing ' + attr + ' in ' + key + ' from ' + value + ' to 1');
                                $child.removeAttr(attr);
                                return;
                            }
                            if (value !== 0.3) {
                                console.log('Changing ' + attr + ' in ' + key + ' from ' + value + ' to 0.3');
                                props[attr] = '.3';
                            }
                        });
                    }
