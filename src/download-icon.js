...
module.exports = (name, theme, icon) => new Promise((fulfill, reject) => {
    let key = name + '-' + theme;
    ...
    if (icon.custom) {
        ...
        // Check for 'baseline-'
        if (theme !== 'baseline') {
            customFile = config.customIconsDir + '/' + icon.local.replace(theme + '.svg', 'baseline.svg');
            try {
                let data = fs.readFileSync(customFile, 'utf8');
                if (typeof data === 'string') {
                    cleanup(data);
    ...
    let suffixes = allSuffixes.slice(0),
        nextSuffix = () => {
            ...
            if (suffix === void 0) {
                console.log(colors.error + '[error]' + colors.reset + ' failed all suffixes for ' + key);
