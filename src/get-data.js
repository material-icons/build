		...
		download({
			...
			.then(meta => {
				...
				if (
					typeof data !== 'object' ||
					data.host === void 0 ||
					data.asset_url_pattern === void 0 ||
					data.icons === void 0 ||
					!(data.icons instanceof Array) ||
				...
				data.icons.forEach(icon => {
					if (icon.unsupported_families.length) {
						...
						errors.push(
							'unsupported_families for icon ' +
								icon.name +
								': ' +
								icon.unsupported_families.join(', ')
						);
					}
				...
				if (config.isOriginalBranch()) {
					...
					let missingJSON;
					try {
						missingJSON = fs.readFileSync(
							config.removedIconsDir + '/data.json',
							'utf8'
						);
					...
					if (oldData.categories !== void 0) {
						...
						if (keys.length) {
							...
							fs.writeFileSync(
								config.removedIconsDir + '/data.json',
								JSON.stringify(missingJSON, null, '\t'),
								'utf8'
							);
						}
						...
						oldData.icons.forEach(icon => {
							if (
								names[icon.name] !== void 0 ||
								missingJSON[icon.name] !== void 0
							) {
								return;
							}
							...
							Object.keys(config.themes).forEach(theme => {
								let source =
										config.outputDir + '/svg/' + name + '/' + theme + '.svg',
									target =
										config.removedIconsDir +
										'/v' +
										version +
						...
						if (missing.length) {
							console.log(
								'Saved backup for missing icons: ',
								missing.join(', ')
							);
