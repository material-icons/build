...
module.exports = icons => new Promise((fulfill, reject) => {
    ...
    let next = () => {
        ...
        downloadIcon(currentKey, currentTheme, currentIcon.themes[currentTheme]).then(html => {
            if (html === null) {
                skip(key);
                return;
            }
            ...
            if (svg.width !== 24 || svg.height !== 24) {
                if (!svg.width || svg.width !== svg.height) {
                    reject('Icon has wrong dimensions: ' + svg.width + ' x ' + svg.height + ': ' + currentIcon.themes[currentTheme].local);
                    return;
                }

                scaleIcon(key, svg).then(svg => {
                    if (svg.width !== 24 || svg.height !== 24) {
                        console.log(svg.toString());
                        reject('Icon has wrong dimensions after scaling: ' + svg.width + ' x ' + svg.height + ': ' + currentIcon.themes[currentTheme].local);
                        return;
                    }
