...
module.exports = (key, svg) => new Promise((fulfill, reject) => {
    ...
    console.log(colors.info + '[' + key + ']' + colors.reset + ': Scaling icon from ' + svg.width);
    tools.Scale(svg, scale).then(svg => {
        let html = svg.toString();

        if (html.indexOf('transform') !== -1 || html.indexOf('<g') !== -1) {
            console.log(colors.info + '[' + key + ']' + colors.reset + ': ' + html);
        }
